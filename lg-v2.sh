#!/bin/bash
#how to use: view in lg.sh in this repo
#SUFFIX=`date +%Y-%m-%d`
rm /home/anhnguyen80/Desktop/source/dirs.txt #tự động cập nhật các repo mới
...
echo -e "$FIND_DIRS"
if [ ! -f "$FILEN" ]; then
  echo "[Warning] Library file not found! Creating new library file"
	touch "$FILEN"
	mkdir -p "$TMP_DIR"
